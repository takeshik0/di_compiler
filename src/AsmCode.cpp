...
std::string
AsmCode::calculateExpression(std::vector<Token>::iterator &currentElement,
                             codeMap &asmCode) {
...
codeMap AsmCode::convertToAsm(std::vector<Token> &tokenList) {
  ...
  for (auto currentElement = tokenList.begin();
       currentElement != tokenList.end(); ++currentElement) {
    if (currentElement->type == TokenType::KwPrint) {
      ...
      asmCode["_start:"].push_back("mov edx, " +
                                   std::to_string(msgToPrint.length()));
